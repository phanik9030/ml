...
def identify_tense(word):
    tokens = nltk.word_tokenize(word)
    tagged = nltk.pos_tag(tokens)
    tense = {
        "past": len([word for word, tag in tagged if tag in ["VBD", "VBN"]]),
        "present": len([word for word, tag in tagged if tag in ["VBP", "VBZ", "VBG"]])
    }
    if tense["past"] > 0:
        return "past"
    elif tense["present"] > 0:
        return "present"
    else:
        return "unknown"
...
# Identify tense of words
tenses = {word: identify_tense(word) for word in words}
print("Tenses:")
for word, tense in tenses.items():
    print(f"{word}: {tense}")
